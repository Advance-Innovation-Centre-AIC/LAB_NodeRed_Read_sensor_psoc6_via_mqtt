...
/**************************************** Example code END******************************/
			...
			//
			//
			//					//				publish_info.payload = "test sending msg";
			//
			//
			//					   xensiv_dps3xx_init();
			//
			...
			//						   xensiv_dps3xx_read(&pressure_sensor, &pressure, &temperature);
			//
			//						   char temp[20]; //create an empty string to store number
			//						   sprintf(temp, "%f", temperature);//make the number into string using sprintf function
			//						   //					printf("You have entered: %s", temp);
			//						   printf("*********TEST function sending sensor value: %s \r\n",temp);
			//						   cyhal_system_delay_ms(1000);
			//
			////						   publish_info.payload = publisher_q_data.data ;
			//						   publish_info.payload = temp;
			//						   publish_info.payload_len = strlen(publish_info.payload);
			//
			//						   printf("  Publisher: Publishing '%s' on the topic '%s'\n\n",
			//								   (char *) publish_info.payload, publish_info.topic);
